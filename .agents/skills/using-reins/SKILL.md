...
## Reins CLI Quick Reference

Bare `reins` (no args) shows a live status home view (branch, active plans,
overlap), not usage — use `reins help` / `reins --help` for the manual.

| Command | Purpose |
|---|---|
| `reins kb sync` | Pull latest kb state |
| `reins kb publish` | Push kb changes (rebase + push) |
| `reins kb status` | Kb health, active plans, overlap, stale docs |
| `reins kb status --compact` | ≤10-line dashboard for agent context (session-start hook) |
| `reins kb lint` | Run kb lint rules |
| `reins kb list` | List repo scopes in the kb |
| `reins kb remove-scope <name>` | Remove a repo scope |
| `reins kb git <args...>` | Raw git passthrough inside the kb |
| `reins spec create "title"` | Create a spec (implementation contract) |
| `reins prd create "title"` | Create a product requirements doc |
| `reins debt create "title"` | Create a tech-debt doc |
| `reins idea create "text"` | Capture an idea |
| `reins <spec\|prd\|debt\|idea> show <slug> [--full]` | Read a kb doc, truncated preview (`--full` for all) |
| `reins <spec\|prd\|debt\|idea> list` | List kb docs of that type |
| `reins plan create` | Create execution plan for current branch |
| `reins plan status` | Plan status for current branch |
| `reins plan show [branch] [--full]` | Show plan doc, truncated preview (`--full` for all) |
| `reins plan complete [branch]` | Archive plan to completed/ |
| `reins retro create` | Create retro for current branch |
| `reins retro show [branch] [--full]` | Show retro doc, truncated preview (`--full` for all) |
| `reins principle add "title"` | Append a golden principle |
| `reins mode enable` / `disable` / `incognito` / `status` | Mode toggles |
| `reins init [...]` | Set up reins in this repo |
| `reins hooks install` | Install git and editor hooks |
| `reins update [--diff X]` | Re-sync bundled files (skills, hooks, AGENTS.md) to the installed reins version |
| `reins feedback [text]` | Open a GitHub issue on the reins tool repo itself (bug/idea about reins, not your project) |
